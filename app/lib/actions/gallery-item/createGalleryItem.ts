...
export async function createGalleryItem(data: { url: string; filename: string; type: 'image' | 'video' }) {
  ...
  try {
    ...
    const message = await buildLogMessage(`uploaded ${data.type} "${data.filename}" to gallery`, actor, context)
    ...
    const message = await buildLogMessage(`failed to save gallery item "${data.filename}"`, actor, context)
