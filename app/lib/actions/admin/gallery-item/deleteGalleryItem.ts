...
export async function deleteGalleryItem(id: string) {
  ...
  try {
    ...
    const message = buildLogMessage(`deleted ${item.type} "${item.filename}" from gallery`, actor, context)
    ...
    const message = buildLogMessage(`failed to delete gallery item (${id})`, actor, context)
