...
export async function updateEndorsement(
  ...
  try {
    ...
    const message = await buildLogMessage(`updated endorsement from ${endorsement.name}`, actor, context)
    await createLog('info', message, { endorsementId: id, ...context })

    return { success: true, data: endorsement }
  } catch (error) {
    console.error('[updateEndorsement]', error)
    const message = await buildLogMessage(`failed to update endorsement (${id})`, actor, context)
