...
import { emailBlastTemplate } from '../../email-templates/admin/emailBlastTemplate'
...
export async function sendVolunteerBlast({
  ...
  try {
    ...
    const message = await buildLogMessage(`sent volunteer blast to ${sent} recipients`, actor, context)
    ...
    console.error('[sendVolunteerBlast]', error)
    const message = await buildLogMessage('failed to send volunteer blast', actor, context)
